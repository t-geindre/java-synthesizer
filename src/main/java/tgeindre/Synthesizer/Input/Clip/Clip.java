...
import tgeindre.Synthesizer.Input.Producer;
...
public class Clip implements Producer {
    ...
    public ArrayList<Message> pullMessages(double time)
    {
        ...
        while (index < maxIndex) {
            ...
            if (note.getAt() < time) {
                messages.add(new tgeindre.Synthesizer.Input.Clip.Message(note.getNote(), true));
                notesOn.add(note);
                index++;
            }
            break;
        ...
        for (Note note: notesOn) {
            if (note.getAt() + note.getDuration() < time) {
                messages.add(new tgeindre.Synthesizer.Input.Clip.Message(note.getNote(), false));
                notesOff.add(note);
            }
        }
